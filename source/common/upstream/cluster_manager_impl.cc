...
bool ClusterManagerImpl::addOrUpdateCluster(const envoy::config::cluster::v3::Cluster& cluster,
  ...
  if (!all_clusters_initialized) {
    ...
    // fixfix post now comment
    postThreadLocalClusterUpdate(*cluster_entry, ThreadLocalClusterUpdateParams());
...
void ClusterManagerImpl::postThreadLocalClusterUpdate(ClusterManagerCluster& cluster,
  ...
  tls_.runOnAllThreads([info = cluster.cluster().info(), params = std::move(params),
                        add_or_update_cluster,
                        load_balancer_factory](ThreadLocalClusterManagerImpl& cluster_manager) {
    if (add_or_update_cluster) {
